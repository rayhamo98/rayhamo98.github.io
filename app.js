...
// =====================
// 5. Galaxy Starfield + Meteors + Floating Robots (clean single loop)
// =====================
const canvas = document.getElementById("sky");
const ctx = canvas.getContext("2d");
...
function drawRobots() {
  for (const b of robots) {
    ...
    // draw (two squares, slight rotation)
    ctx.save();
    ctx.translate(b.x, b.y);
    ctx.rotate(Math.sin(b.spin) * 0.25);
    ctx.strokeStyle = `hsl(${b.hue}, 100%, 70%)`;
    ctx.globalAlpha = 0.35 + 0.2 * Math.sin(b.spin * 2);
    ctx.lineWidth = 2;
